...
export function normalizeEvidence(observations=[]){
 const accepted=[],duplicates=[],rejected=[],seen=new Set();
 for(const observation of observations){const firmware=text(observation?.firmware),sessionId=text(observation?.sessionId);if(!FIRMWARES.includes(firmware)||!sessionId){rejected.push({firmware,sessionId,reason:'unsupported firmware or missing sessionId'});continue;}const key=sessionKey(observation);if(seen.has(key)){duplicates.push({firmware,sessionId});continue;}seen.add(key);accepted.push(observation);}
