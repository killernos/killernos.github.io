...
function snapshot(overrides={}){return {sessionId:'NEXT-SESSION-test-12',firmware:'13.02',firmwareSource:'UA',hardware:'hardware',simulation:false,buildId:'test',carrierObtained:true,windowPInstalled:true,readVerified:true,writeVerified:true,userlandARWVerified:true,events:[{stage:'NEXT-1302-HARDWARE-CONFIRMED'}],...overrides};}
const good=buildObservationArtifact(snapshot());
assert.equal(good.ok,true);assert.equal(good.artifact.reportType,'NEXT_HARDWARE_OBSERVATION_SESSION');assert.equal(good.artifact.evidenceClass,'HARDWARE_OBSERVED');assert.equal(good.artifact.kernelExecutionObserved,false);assert.equal(good.artifact.kernelWriteObserved,false);assert.equal(good.artifact.henObserved,false);
