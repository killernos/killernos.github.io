import { PS4 } from "../runtime/next/ps4_offsets.js";

const CONFIGURED_KEYS = Object.keys(PS4).sort(function (left, right) {
...
function normalizeFirmware(value) {
  ...
  const parsed = /^(\d+)(?:\.(\d{1,2}))?$/.exec(clean);
  let minor;
  if (!parsed) return "";
  minor = parsed[2] || "00";
...
function backendNameFor(entry, firmware) {
  const status = String(entry && entry.fw_status || "");
...
function getNextFirmwareCapability(firmware) {
  ...
  const exact = normalized ? PS4[normalized] || null : null;
